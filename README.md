...
✔️ **Rastreabilidade Financeira** -- Cada operação gera uma transação
com tipo, valor, saldo antes/depois e timestamp.
✔️ Registro histórico imutável
✔️ Separação entre lógica financeira e persistência

Cada operação gera uma transação contendo:
...
O estado do usuário é persistido no banco, garantindo continuidade da conversa.

----------------------------------------------------------------------

## 🛡 Tratamento de Erros

O sistema utiliza uma exceção padronizada `AppException` contendo:

-   Código estruturado de erro  
